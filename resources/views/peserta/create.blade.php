...
<section class="main-section">
    ...
    <div class="content">
        ...
        <div class="row">
            ...
            <form action="{{route('peserta.store')}}" method="POST" class="form-horizontal" role="form">
            {{ csrf_field() }}
                ...
                <div class="form-group {{ $errors->has('jk') ? ' has-error' : '' }}">
                    ...
                    <select name="jk" class="form-control" required="reuired">
                        ...
                        @foreach($jks as $jk => $val)
                            <option value="{{$jk}}" @if(old('jk')==$jk) selected @endif >{{$val}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('nama_rt') ? ' has-error' : '' }}">
                    ...
                    <select name="nama_rt" class="form-control">
                        ...
                        @foreach($rts as $rt => $val)
                            <option value="{{$rt}}" @if(old('nama_rt')==$rt) selected @endif >{{$val}}</option>
                        @endforeach
                    </select>
